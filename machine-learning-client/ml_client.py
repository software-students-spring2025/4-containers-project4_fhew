...
def run_analysis():
    ...
    user_location = {"latitude": 40.5412, "longitude": -74.1515}  # sample location in Staten Island

    stations = load_station_data(CSV_PATH)
    near_by_stations = find_near_by_stations(user_location["latitude"], user_location["longitude"], stations)
...
def find_near_by_station(user_lat, user_lon, radius_km=5):
...
def visualize_stations(user_location, nearby_stations, radius_km=5, image_name="map.png", output_dir="/app/static"):
    """ Generate and save a map image of the user location and nearby fire stations. """
    fig, ax = plt.subplots()

    # Plot user
    ax.scatter(user_location["longitude"], user_location["latitude"], color='red', label='User Location', zorder=5)
    # Plot nearby fire stations
    for station in nearby_stations:
        ax.scatter(station["longitude"], station["latitude"], color='blue', zorder=3)
        ax.text(station["longitude"], station["latitude"], station["station_name"][:20], fontsize=6)
    # Radius circle
    radius_deg = radius_km / 111
    circle = plt.Circle((user_location["longitude"], user_location["latitude"]), radius_deg,
                        color='gray', fill=False, linestyle='--', label=f"{radius_km}km radius")
    ax.add_patch(circle)
